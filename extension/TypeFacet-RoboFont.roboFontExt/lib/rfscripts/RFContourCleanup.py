...
def processGlyph(glyph, glyphName, paths):
#    print
#    print 'processGlyph'
#    for path in paths:
    ...
    for path in paths:
        ...
        for segment in path.segments:
            if len(segment.points) == 4:
                ...
                if p0 == p1 and p2 == p3:
                    print '\tRemoving unnecessary bezier', glyphName
                    segment = TFSSegment(p0, p3)
            newSegments.append(segment)
        newPaths.append(TFSPath(True, *newSegments))
    return newPaths


def cleanupGlyphContours(glyph):
    glyph = TFSGlyph(glyph)
...
def cleanupFontContours(font):
    glyphs = [glyph for glyph in font]
    glyphs.sort(lambda g0,g1: cmp(g0.unicode, g1.unicode))
